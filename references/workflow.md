# koko-ship — Post Generation Workflow

This file contains the step-by-step post generation pipeline. It is loaded by the writer skill when generating content. For the skill's core identity, voice layers, and cardinal rules, see the writer skill.
...
Before generating anything, check for unprocessed edit diffs in `voice/changelog.md`. Read `<cwd>/.bip-history.json`, find entries where `was_edited: true` AND `diff_processed: false`, extract patterns from the diff, and log them in `voice/changelog.md`. Mark as processed. After 5+ entries with recurring patterns (3+ occurrences), propose updates to `voice/profile.md` → Learned patterns section with user approval.
...
Read all 3 voice files. Each serves a different purpose during generation:

- **`voice/profile.md`** — signature patterns, anti-patterns, evaluator checklist, vocabulary markers, sentence rhythm, emotional expression. This is the primary drafting reference.
- **`voice/marketing-voice.md`** — post characteristics (language, target length, line format, emoji rules, hashtag rules), publishable filter, content angles. This governs output formatting.
- **`voice/patterns.md`** — thinking style, problem approach, reaction patterns. Used for moment selection and angle choice — not for direct pattern checking, but for understanding which moments this person would naturally share and how they'd frame them.

When voice files and writing principles conflict, **voice wins**. The principles define WHAT to write (structure). The voice profile defines HOW to write (tone, vocabulary, emoji, length, hashtags).

### Step 3: Check history (avoid repetition)

If `<cwd>/.bip-history.json` exists, read it and:
- Note `context_lines_used` — you must write a **new** context line from a different angle (see `~/.claude/skills/koko-ship-shared/references/writing-principles.md` → CONTEXT LINE VARIATION)
...
```bash
python3 ~/.claude/skills/koko-ship-shared/scripts/read_project_sessions.py --cwd "$(pwd)" --hours 24 --fallback-hours 168
```
...
1. **User's own reaction** (most authentic) — look for the user expressing surprise, frustration, realization. Phrases like "wait what", "oh that's why", "huh", or explicit diagnosis ("this is a bug", "i designed this wrong"). This is the best WHY source because it's the user's actual words.

2. **Claude's diagnosis** — look for Claude explaining what went wrong or why something happened. Technical explanations, root cause analysis, or debugging conclusions. Good WHY source but needs to be translated into the user's voice for the post.

3. **The fix / resolution** — look for what changed after the problem was identified. Code changes, design changes, the user asking Claude to fix it. The fix implies the WHY.

**Minimum: at least 1 WHY source must exist for a moment to be postable.** If you find a result but no WHY in the session (no reaction, no diagnosis, no fix), the moment is incomplete — flag it to the user: "i see [result] but can't find why it happened in the session. what was the reason?"
...
### Step 5: Apply security + platform filters

Read `~/.claude/skills/koko-ship-qa/SKILL.md` and apply all checks: security filter, platform rules, jargon check, repetition check, privacy check.

### Step 6: Apply writing principles

Read `~/.claude/skills/koko-ship-shared/references/writing-principles.md`. There are no rigid templates — apply these 5 principles and let the post take whatever shape fits the moment:
...
Read `~/.claude/skills/koko-ship-editor/SKILL.md` and evaluate all 3 drafts from Step 7 against both levels: Voice Accuracy and Content Quality.

This is the single most important step — it is what separates koko-ship from generic AI output.

**Pick the winner:**
- Among drafts that pass all gates, pick the one with the highest combined score
...
**Auto-capture:** Run `python3 ~/.claude/skills/koko-ship-shared/scripts/capture_screenshot.py --detect --cwd "$(pwd)"` to find capturable sources. If a running dev server or HTML file exists, capture a screenshot or short video. Always prefer live URL over static HTML. Save to `.bip-images/`.
...
```bash
python3 ~/.claude/skills/koko-ship-shared/scripts/generate_image.py --slug <post-slug> --json '<json-payload>'
```
...
```json
{
  ...
  "subtitle": "fed writing patterns into one ruleset",
  "stats": [
...
If the file doesn't exist, create it with the schema in `~/.claude/skills/koko-ship-shared/references/history-schema.json`.

**Voice evolution:** If `was_edited: true`, the diff will be analyzed at the start of the next session (Step 0.5). This keeps the current session fast — no analysis during the approval flow.
