...
After users edit AI-generated posts, the skill captures diffs and extracts patterns (see `references/voice-evolution-v2.md`). Once 3+ occurrences of a pattern are confirmed by the user, they get added to `~/.bip-voice.json` → `learned_patterns`.

When evaluating, read `learned_patterns` from the user's voice profile JSON:
- `learned_patterns.must_have` — treat as must-have in the checklist
- `learned_patterns.never_use` — treat as never-use (auto-fail if present)
- `learned_patterns.preferences` — treat as enhancers (boost score when present)
...
Lives at: `references/evaluate.md` (inside the skill folder)
